...
class DataService {
    ...
    constructor(data) {
        ...
        this.onStateChanged = () => {
            ...
            console.log("Patch locations", patches.map((delta) => delta.pointer));
            ...
            for (let i = 0, l = patches.length; i < l; i += 1) {
                ...
                // @obsolete changes should be replaced by patch
                const changes = getChangeDetails(lastUpdate, current.data.present);
                this.emit(EVENTS.AFTER_UPDATE, eventLocation, { type: parentDataType, changes, patch: patches[i].patch });
                this.bubbleObservers(eventLocation, { type: parentDataType, changes, patch: patches[i].patch });
            }
        ...
        //     console.log("Update location", eventLocation);
        //     console.log("Patch locations", patches.map((delta) => delta.pointer));

        //     DEBUG && console.log("DataService notify update", eventLocation, changes);
    ...
    set(pointer, value, isSynched = false) {
        ...
        state.dispatch(ActionCreators.setData(pointer, value, currentValue, isSynched));

        if (pointer === "#" && isSynched === false) {
            // do not add root changes to undo
            state.get(this.id).data.past.pop();
        }
    }
