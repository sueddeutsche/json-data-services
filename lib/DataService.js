...
class DataService {
    ...
    constructor(data) {
        ...
        state.subscribe(this.id, () => {
            ...
            // perform diff and send update on single value only
            const diff = deepdiff.diff(lastUpdate, current.data.present);
            if (diff && diff.length === 1 && diff[0].kind === "E" && diff[0].path) {
                action.pointer = `#/${diff[0].path.join("/")}`;
                // console.log("selective update", action.pointer);
            } else {
                // console.log("ignore diff", diff);
            }
            ...
            if (action.pointer === "#") {
                ...
                console.log("update pointer", action.pointer);
            }
            */
